...
const PAID_TAG = 'Astro Paid';
const CONSULTATION_AMOUNT_FIELD = 'cf_consultation_amount';

function normalizeContactTags(tags) {
  ...
  return tags
    .map(tag => {
      ...
      if (tag && typeof tag === 'object') {
        return String(tag.name ?? tag.value ?? tag.label ?? '').trim();
      }
      return '';
    })
    .filter(Boolean);
...
async function updatePaidContact(contactId, amount) {
  required('FRESHSALES_BUNDLE_ALIAS', FRESHSALES_BASE);
  required('FRESHSALES_API_KEY', FRESHSALES_API_KEY);

  const contact = await getContactById(contactId);
  if (!contact) throw new Error(`Freshsales contact ${contactId} was not found.`);

  const existingTags = normalizeContactTags(contact.tags);
  const tags = existingTags.some(tag => tag.toLowerCase() === PAID_TAG.toLowerCase())
    ? existingTags
    : [...existingTags, PAID_TAG];

  const numericAmount = Number(amount);
  if (!Number.isFinite(numericAmount) || numericAmount <= 0) {
    throw new Error(`Invalid consultation amount: ${amount}`);
  }
    ...
    `${FRESHSALES_BASE}/api/contacts/${encodeURIComponent(contactId)}`,
    {
      ...
      body: JSON.stringify({
        contact: {
          tags,
          custom_field: {
            [CONSULTATION_AMOUNT_FIELD]: numericAmount
          }
        }
      })
    }
  ...
  if (!response.ok) {
    throw new Error(
      `Freshsales paid contact update error (${response.status}): ${JSON.stringify(data)}`
    );
  }
